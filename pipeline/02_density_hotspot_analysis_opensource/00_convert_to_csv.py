#!/usr/bin/env python3
r"""提取出租车轨迹的核心字段，并由 TSV 转换为 CSV。

这是整套pipeline的第0阶段(原始13列TSV -> 本目录01脚本吃的core.csv)，本来就不依赖
arcpy(纯csv/heapq/tempfile标准库实现，外部归并排序)，跟01~09一样可以直接在没有
ArcGIS授权的云端服务器上跑。产出的core.csv已经按真正唯一的车辆id(输出里的taxi_id
列，即原始第2列)+timestamp做好全局排序，供01_feature_engineering.py单遍流式处理，
不需要再排一次序——具体原因见01_feature_engineering.py顶部注释。

用法:
    python 00_convert_to_csv.py 原始13列TSV路径 输出core.csv路径
    python 00_convert_to_csv.py raw/20180301.txt 20180301_core.csv   # 换年份/日期同样适用
"""
